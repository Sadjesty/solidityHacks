...
contract Vault {
    ...
    bool public isTrue;// 1 byte
    uint16 public u16 = 31;// 2 bytes(2**8) * 2
    //slot 2
    bytes32 private password;

    //constants do not use storage (hardcoded in SC bytecode)
    uint public constant SOME_CONST = 123;
    ...
    function getArray(uint slot, uint index, uint elementSize) public pure returns(uint) {
